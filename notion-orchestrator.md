...
若來源網址包含 `bloomberg.com`，使用 Playwright MCP 讀取（`browser_navigate` + `browser_snapshot`）。若仍失敗則標記為失敗，並說明原因（建議編輯改用 gifted 連結）。

#### 2d. 寫回 Notion

所有子代理完成後，**依序逐一**寫回（此步驟仍循序，避免 Notion API 頻率限制）。每篇分兩步寫回：
