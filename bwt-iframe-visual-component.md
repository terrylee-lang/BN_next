...
**每個配件的破折號「——」總數限制為 1 次**（比稿件內文更嚴格）。原因：配件內文都是短句，破折號濫用會稀釋衝擊力。

**保留時機**：用於「反直覺翻轉」或「最有衝擊力的對比」。
- ✅ 「以為程式碼是護城河——其實是累積的領域深度」（翻轉直覺）
- ❌ 「列出採用的模式、禁用的依賴、取捨——每次新會話先讓 Claude 讀」（功能性連接，用句號或逗號）
- ❌ 「把『建構』當『驗證』——42% 新創倒在打造沒人要的東西」（補充說明，用逗號）

**自查指令**：產出 HTML 後跑 `grep -c "——" <file>`，超過 1 立刻精簡。
