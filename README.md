...
| `retrospective.md` | retrospective | 工作覆盤：掃描對話，整理記憶與 Skill 更新建議 |
...
**deep-analysis / draft-polish 完成後**，Claude 會自動判斷是否將研究內容寫回 vault：
...
- Vault 架構需自行建立，不包含在本 repo 安裝流程中
- `lint-vault` skill（vault 健康檢查）為個人工具，未收錄於本 repo
- 詳細使用說明參閱個人 vault 內的 `vault-guide.md`
