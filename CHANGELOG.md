...
**採用主編大通則第 5 條「英、數前後勿加半形空格」，輸出格式範例全面同步**

源頭：編輯部 0907 版用字表大通則第 5 條。盤點 vault 270 份 Claude 產出稿件，中英數相鄰處 33,235 處有空格、2,031 處無空格；官網 9 月 4 日三篇外電標題全部不加空格，摘要兩種並存。Terry 2026-09-08 裁決採用，收斂到編輯部寫法。

### bwt-style-guide.md

- 大通則新增第 5 條：中文與英文、數字直接相連（4月27日、新台幣30億元、1TW、Claude Code上線）；英文詞組內部空格照原文（Claude Code、GPT-6 Astra）。明寫「本規範檔的說明文字不受此限，只有引號內與表格內的範例代表輸出格式」，因為規範檔散文有百餘處空格，屬內部用語不改
- 輸出格式範例同步去空格：大通則 4 幣值單位、SI 單位「1TW（太瓦）」、第 4 節新台幣XXXX元／美國FDA／歐盟CE認證、第 5 節 100間藥局、「日期格式」節全部範例（4月27日、2026年、2026年4月27日、6月1至5日、5月30日至6月2日、截至X月X日）

### article-checker.md

- Step 3 錯字偵測補「中文與英數相連處不得有半形空格」，指向 lint「中英數夾空格」（報總數與前 5 處，期望值 0）
- Step 5 貨幣格式改「新台幣XX億元」，範例句「該公司完成5億美元（約新台幣159億元）的C輪融資」；規則 F 錨點範例「7月8日」「截至7月7日」；規則 E 軟化語氣範例、ARR 里程碑範例同步去空格

### 其他 Skill 輸出範例同步

- news-daily.md 匯率格式 `原始金額（約合新台幣XX億元，匯率參考：XX來源）`
- bwt-html-table-component.md、bwt-iframe-visual-component.md 日期格式「2026年5月12日」
- etf-explainer.md 資料日期範例「2026年5月11日」
- CLAUDE-template.md 注意事項「截至X月X日」

### 個人層同步（不在本 repo）

- `bwt-lint.sh`：新增「中英數夾空格」檢查，只報總數與前 5 處（Claude 舊稿每篇 100 至 250 處，逐行列會淹沒其他項目）；「新台幣寫法」註解同步
- `~/.claude/CLAUDE.md` 注意事項「截至X月X日」

---

## v3.2.9 — 2026-09-08

**主編 0907 版用字表同步：大通則、姓氏譯名、企業後續稱呼、簡語格式，並修正 v3.2.8 一處誤判**

源頭：雜誌主編 2026-09-07 更新編輯部用字表（檔名「統一用字表（0907）」）。注音表與「其他」表主體與 2022 版相同，新增前置「大通則」與「看稿時常改的校對用字」兩段；刪除 PCR、TFDA、IPO、IDM 與 33 家企業清單（改為通則）。

### bwt-style-guide.md

- 節首新增大通則四條：字形從簡（台灣不做臺灣）、教育部字典為準、技術新詞依台灣財經媒體慣用且通篇一致、幣值單位勿漏（新台幣在前、其他貨幣置數字後）。上游第 5 條「英數前後勿加半形空格」與本檔既有寫法相反，待 Terry 裁決，本版未納入
- 第 1 節新增：台／臺、即使／即便、熱中／熱衷、思惟／思維、暴紅／爆紅；「表」列補「手表」（v3.2.8 誤寫「手錶仍用錶」，依上游更正）
- 第 3 節：互聯網對應改「網路」
- 第 4 節新增簡語譯名格式（英文在前中文在後、中文只附第一次、英文全稱可放可不放、大小寫比照官網或維基 logo）；EPS 改「每股盈餘」、COVID 改「新冠肺炎（COVID-19）」、CAGR 改「年均複合成長率」、CDMO 去英文全稱；刪 PCR、TFDA、IPO、IDM 四列
- 第 5 節改寫為企業譯名通則（第一次中（英）全稱、後續只取廣為人知稱呼）＋外國人名只翻姓氏（基辛格、馬斯克；冠夫姓難辨者如希拉蕊例外）；譯名表帕特·基辛格改基辛格、新增臉書；量詞補「一輛車不做一台」
- 「人名與公司名規範」同步：人名範例「伊隆·馬斯克」改「馬斯克 (Elon Musk)」並明寫只翻姓氏；公司名補後續稱呼與大小寫規則
- 加密貨幣：上游 0907 版仍寫「密碼貨幣」，Terry 2026-09-08 再次裁決維持「加密貨幣」（密碼貨幣為舊用法）；渠道／通路欄位上游仍顛倒，本檔維持通路為正

### 個人層同步（不在本 repo）

- `bwt-lint.sh`：字形從簡補即便、熱衷、思維、爆紅、臺灣、錶（全量）、一台＋車；新增「外國人名全名」候選規則（中點連接的中文全名）
- memory「Mark Gurman 中文譯名」改為只寫「古爾曼（Mark Gurman）」

---

## v3.2.8 — 2026-09-07

**編輯部《用字規範／常見錯字一覽表》20220315 版併入 style-guide，lint 腳本同步**

源頭：Terry 指定以編輯部 Google Doc「數位時代用字規範／常見錯字一覽表 20220315 版」為統一用字依據。盤點 vault 270 份 Claude 產出稿件，違反最多的是「週」179 處、「規劃」83 處、「佔」70 處、「一位」14 處，證實規則只寫進 style-guide 不夠，須同步進確定性掃描。

### bwt-style-guide.md：「兩岸用語轉換」節擴為「統一用字與常見錯字（必須執行）」

- 五個小節：① 字形從簡錯字表 34 條（正字 → 錯字單表；上游按注音排序、20 個注音區塊為空，對模型查找無意義，故重排）② 語意辨析組 11 組（紀錄／記錄、雇／僱、必須／必需等，lint 掃不出、須逐處判讀）③ 兩岸用語（原 10 條＋新增 11 條：智能、移動、互聯網、軟件、硬件、計算機、攝像頭、激光、初創、渠道、納斯達克；SI 前綴譯法原地保留）④ 專名、縮寫與貨幣寫法（App、Wi-Fi、Fintech、Ptt、MP3、美元、加密貨幣、FDA／TFDA／CDMO／EPS／GMP 全名寫法、財星、富比士）⑤ 譯名對照 22 組、直接用英文清單、量詞
- 節首以 HTML 註解標上游文件網址、版本與同步日期；上游改版時整節重比對，不逐條補丁
- Terry 裁決（2026-09-07）：「週」全量改「周」（含每周、本周等時間週期義）；「規劃」全量改「規畫」；「一位」改「一名」；「加密貨幣」為統一寫法（上游原訂「密碼貨幣」）；CAGR 譯「年複合成長率」（上游「複合年均增長率」偏大陸用語）
- 修正上游原檔錯誤：「渠道／通路」欄位左右顛倒（通路為正）、NIVIDIA 拼寫、汰除 2022 年「新冠狀病毒（俗稱武漢肺炎）」寫法；公司名順序統一依「人名與公司名規範」中文為主（上游混用兩種順序）
- 本檔自身用字同步：「上週」→「上周」、「公告後一週內」→「一周內」、「大綱規劃階段」→「規畫階段」

### article-checker.md：Step 3 錯字偵測指向新表

- 前置讀取與 lint 說明補「統一用字（含兩岸用語）」「常見錯字／專名格式」；Step 3 明寫第 1、4 節由 lint 掃候選後逐項確認、第 2 節語意辨析組須逐處判讀
- 規則 F 相對時間詞搜尋清單改「本周」「上周」，舊寫法同查
- 本檔自身用字同步：週活→周活、上週四→上周四、產品規劃→產品規畫、一位分析師→一名分析師

### 個人層同步（不在本 repo，交接者自行對應）

- `~/.local/bin/bwt-lint.sh`：新增「常見錯字（字形從簡）」「常見錯字（語境判斷）」「專名與縮寫格式」三條規則；兩岸用語正規表達式補 11 詞；相對時間詞補「本周／上周」
- `~/.claude/CLAUDE.md` 產稿 Skill 必讀行同步為「統一用字與常見錯字表（含兩岸用語）」

### 交接文件

- README.md 檔案表、CLAUDE-template.md、bn-claude-code-init.md、longform.md 規範對照表：style-guide 描述補「統一用字與常見錯字」；bwt-html-table-component.md、bwt-iframe-visual-component.md、bwt-visual-checklist.md、bwt-design-standard.md、etf-explainer.md 的 style-guide 對照清單同步加列

---

## v3.2.7 — 2026-09-04

**體檢糾正判讀長出的三條規則：口徑修正不加免責句、屬性欄同步、冗長列為 AI 味**

源頭：系統體檢（個人層 `ops/diagnosis-2026-09.md`）判讀 2026-08-05～09-04 的 50 筆糾正命中，34 筆為真。兩個發現要進規範：(1) 7 月第一名模式「寫太滿」仍在，攔截點從編輯移到 Codex；而 Codex 精度輪之後 Claude 反向補償、全文加滿限定語，Terry 明說「不喜歡口徑澄清，查證過確認沒錯就好」。(2)「冗長／廢話」12 筆成為第二大類（7 月無此類），集中在教學文（OpenClaw 教學文被批「整篇都在廢話，重寫算了」）與會員專刊（實作過程過細、全刊精簡 15%）；AI 味總表原本只管句式，管不到重複觀點與過度細節。另一筆流程漏洞：修內文後 Notion 摘要／重點屬性欄未同步，編輯多補一輪才一致。

### article-checker.md：「Codex 協作協議」執行紀律補兩條

- **口徑修正不等於加免責句**：只改被指出那一處的證據級別用語（依規則 E 五級表），不在全文他處補「可能／據稱／尚待確認」；修正後全文限定語數量不得高於修正前
- **屬性欄同步**：修訂動到摘要／重點所依據的事實或數字時，同一輪把 Notion 屬性欄改到與內文一致，不留舊版

### bwt-style-guide.md：AI 味句式總表新增「重複觀點與過度細節」列

- 上限：同一觀點全文只出現一次；實作過程只寫到讀者能重現（保留使用者下的 prompt 與調整原因，刪工具中間交付）；功能列舉每項一句、不超過三項
- 「替代寫法」表對應加一列：實作段改成「做了什麼→看到什麼→調整了什麼」三句一組
- 落點選總表而非只改 longform：article-checker 依總表掃全稿種，冗長不是步驟型獨有（專刊 5 筆是論證與實測的混合體）

### longform.md：步驟型補「實作精簡」

- 品質 rubric 新增「實作精簡（步驟型）」列；出稿前 checklist 第 11 項補「實作段是否只寫到能重現、有無工具中間交付與除錯往返寫進正文、同一觀點是否只出現一次」。規則本體指向 style-guide 新列，本檔只補步驟型的操作口徑

### 未改動（刻意）

- `deep-analysis.md`、`tutorial-article.md`：過渡期退路、預計廢除，靠 style-guide 總表覆蓋
- 會員專刊本身的動線（PDF 給路徑不給附檔、每工作天新 session、status 區塊格式、文末採條列）屬 Terry 個人流程，寫在 vault `.claude-refs/member-magazine-sop.md`，不進本 repo
